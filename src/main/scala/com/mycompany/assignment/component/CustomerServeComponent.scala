...
trait CustomerServe{
    def getTotalTime(customers: List[Customer]): Int
    def averageWaitTime(customers: List[Customer]): Int
}
...
object CustomerServeComponent extends CustomerServe {
    ...
    override def getTotalTime(customers: List[Customer]): Int = {
        var totalTime = 0
        var currentTime = 0

        val customersInTimeOrder: ListBuffer[Customer] = customers.sortWith(_.orderTime < _.orderTime).to[ListBuffer]
    ...
    override def averageWaitTime(customers: List[Customer]): Int ={
        getTotalTime(customers) / customers.size
    }
